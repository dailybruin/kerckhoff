      ...
      <b-tab v-for="packageSet in packageSets" :key="packageSet.slug" :title="packageSet.slug">
        ...
        >
          <template slot="slug" slot-scope="data">
            <b-link :to="'/manage/packages/' + packageSet.slug + '/' + data.value">{{data.value}}</b-link>
          </template>
          <template slot="gdrive_url" slot-scope="data">
            <a :href="data.value" target="_blank">Google Drive Folder</a>
          </template>
        </b-table>
        <b-pagination :total-rows="totalRows" v-model="currentPage" :per-page="30"></b-pagination>
      </b-tab>
      ...
      <b-form ref="packageForm" @submit="submitForm">
        <div class="container" fluid>
          ...
          >
            ...
            ></b-form-input>
            <b-form-invalid-feedback v-if="errs.slug">{{errs.slug[0]}}</b-form-invalid-feedback>
          </b-form-group>
          ...
          >
            ...
            ></b-form-input>
            <b-form-invalid-feedback v-if="errs.description">{{errs.description[0]}}</b-form-invalid-feedback>
          </b-form-group>
          ...
          >
            ...
            ></b-form-input>
            <b-form-invalid-feedback v-if="errs.drive_folder_url">{{errs.drive_folder_url[0]}}</b-form-invalid-feedback>
          </b-form-group>
          ...
          >
            ...
            ></b-form-input>
            <b-form-invalid-feedback v-if="errs.publish_date">{{errs.publish_date[0]}}</b-form-invalid-feedback>
          </b-form-group>
        </div>
      </b-form>
      <div slot="modal-footer" class="w-100">
        <b-btn variant="primary" v-if="!submitted" @click="submitForm">Submit</b-btn>
        <h4 v-else>Creating...</h4>
      </div>
    </b-modal>
  </div>
</template>
  ...
  computed: {
    hasError: function() {
      for (let err in this.errs) {
        if (this.errs[err] != null) console.log("err");
        return true;
      }
      console.log("no err");
      return false;
  ...
  methods: {
    ...
    getPackageData: function(ctx) {
      let promise = axios.get(
        "/api/packages/" + this.packageSet + "?page=" + this.currentPage
      );
    ...
    submitForm: function(evt) {
      ...
      this.submitted = true;
      let res = axios
        .post("/api/packages/" + this.packageSet, this.form)
        .then(res => {
          console.log(res);
          this.form = {
            slug: "",
